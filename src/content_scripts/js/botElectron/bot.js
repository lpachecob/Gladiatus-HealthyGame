...
function noplay() {
  return (
    (localizacion.includes("guild") && !localizacion.includes("guildTemple")) ||
...
function autopray() {
  ...
  if (posicion >= 0) {
    prayurl =
      prayurl.slice(0, posicion) + "guildTemple" + prayurl.slice(posicion + 8);
  }
  ...
  if (localizacion == questurl) {
    if (questrestart.length > 0) {
      ...
      for (let i = 0; i < questdiv.length; i++) {
        let questtype = questdiv[i].querySelector("div.quest_slot_icon").style
          .backgroundImage;
        if (
          questtype == 'url("9379/img/ui/quest/icon_dungeon_inactive.jpg")' ||
