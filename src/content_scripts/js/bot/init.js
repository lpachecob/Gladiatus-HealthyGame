export class Bot {
  static run() {
    ...
    function noplay() {
      return (
        (localizacion.includes("guild") &&
          !localizacion.includes("guildTemple")) ||
        localizacion.includes("mod=player") ||
        localizacion.includes("mod=highscore") ||
        localizacion.includes("mod=recruiting") ||
        localizacion.includes("mod=powerups") ||
        localizacion.includes("mod=premium") ||
        localizacion.includes("mod=missions") ||
        localizacion.includes("mod=gods") ||
        localizacion.includes("mod=mysterybox") ||
        localizacion.includes("mod=overview") ||
        localizacion.includes("mod=messages") ||
        localizacion.includes("mod=packages") ||
        localizacion.includes("mod=news") ||
        localizacion.includes("mod=surveys") ||
        localizacion.includes("mod=settings") ||
        localizacion.includes("mod=memo") ||
        localizacion.includes("mod=stuff") ||
        localizacion.includes("mod=auction") ||
        localizacion.includes("mod=inventory") ||
        localizacion.includes("mod=forge") ||
        localizacion.includes("mod=magus") ||
        localizacion.includes("mod=market") ||
        localizacion.includes("mod=training") ||
        localizacion.includes("mod=hermit") ||
        localizacion.includes("mod=costumes") ||
        localizacion.includes("mod=craps")
      );
    }
    ...
    var workurl = document.querySelector(
      "div#submenu1 a.menuitem:not(.eyecatcher)"
    ).href;
    execute();
    function execute() {
      if (!noplay()) {
        ...
        if (boton) {
          if (
            ...
            if (localizacion == workurl) {
              try {
                let cancelar = document.querySelector(
                  "div#content article section table tbody tr td a"
                );
                setCookie("working", false, 97);
                cancelar.click();
                setTimeout(function () {
                  let aceptarcancel = document.querySelector(
                    "td#buttonleft input"
                  );
                  aceptarcancel.click();
                }, 1000);
    ...
    function jugar() {
      let cooldownexpedition = document
        .querySelector("#cooldown_bar_fill_expedition")
        .getAttribute("style");
      let expeditionpoints = parseInt(
        document.querySelector("#expeditionpoints_value_point").innerHTML
      );
      let cooldowndungeon = document
        .querySelector("#cooldown_bar_fill_dungeon")
        .getAttribute("style");
      let dungeonpoints = parseInt(
        document.querySelector("#dungeonpoints_value_point").innerHTML
      );
      let cooldownarena = document
        .querySelector("#cooldown_bar_fill_arena")
        .getAttribute("style");
      let cooldownturma = document
        .querySelector("#cooldown_bar_fill_ct")
        .getAttribute("style");
      ...
      if (boton) {
        if (
          autoexpeditionok &&
          cooldownexpedition == "width: 100%;" &&
          !hpLowerThan(expeditionhp)
        ) {
          autoexpedition();
    ...
    function autodungeon() {
      ...
      if (elemFights.length > 0) {
        if (skipboss) {
          try {
            let jefelabel = document
              .querySelector("div.map_label")
              .innerText.slice(-1); /////////////////////////////NO JEFE
            if (
              jefelabel != "0" &&
              jefelabel != "1" &&
              jefelabel != "2" &&
              ...
              let cancelar = document.querySelector(
                "#content div:nth-child(6) form input.button1"
              );
              cancelar.click();
            }
          } catch (error) {
    ...
    function autoexpedition() {
      ...
      let selectedexpeditiontarget = parseInt(
        getCookie("autoexpeditiontarget")
      );
      if (
        !localizacion.includes("location") ||
        localizacion.includes("submod")
      ) {
        ...
        attack(
          null,
          "" + selectedexpeditionmap + "",
          selectedexpeditiontarget,
          0,
          ""
        );
      }
    ...
    function autoarena() {
      ...
      if (
        localizacion.includes("mod=arena") &&
        !localizacion.includes("serverArena")
      ) {
        window.location = document.querySelector(
          "ul#mainnav li table tbody tr td:nth-child(2) a"
        ).href;
        setTimeout(myPause, 5000);
      } else if (
        !localizacion.includes("serverArena") ||
        !localizacion.includes("Type=2")
      ) {
        ...
        for (let i = 1; i < 6; i++) {
          let lvl = parseInt(
            document
              .querySelectorAll("section#own2 table tbody tr")
              [i].querySelectorAll("td")[1].innerHTML
          );
          if (
            lvl < limit &&
            (last == 0 ||
              (lvl >
                parseInt(
                  document
                    .querySelectorAll("section#own2 table tbody tr")
                    [last].querySelectorAll("td")[1].innerHTML
                ) &&
                !document
                  .querySelectorAll("section#own2 table tbody tr")
                  [i].querySelectorAll("td a")[0]
                  .innerHTML.includes("ByElection")))
          ) {
            last = i;
          }
        }
        if (last == 0) {
          if (
            document
              .querySelectorAll("section#own2 table tbody tr")[1]
    ...
    function autoturma() {
      ...
      if (localizacion.includes("grouparena")) {
        window.location = document.querySelector(
          "ul#mainnav li table tbody tr td:nth-child(4) a"
        ).href;
        setTimeout(myPause, 5000);
      } else if (
        !localizacion.includes("serverArena") ||
        !localizacion.includes("Type=3")
      ) {
        ...
        for (let i = 1; i < 6; i++) {
          let lvl = parseInt(
            document
              .querySelectorAll("section#own3 table tbody tr")
              [i].querySelectorAll("td")[1].innerHTML
          );
          if (
            lvl < limit &&
        ...
        if (last == 0) {
          if (
            document
              .querySelectorAll("section#own3 table tbody tr")[1]
              .querySelectorAll("td a")[0]
              .innerHTML.includes("ByElection")
          ) {
            last = 2;
        ...
        if (!localizacion.includes(eventboton.href)) {
          ...
          try {
            let targets = document.querySelectorAll(
              "div#expedition_list button.expedition_button:not(:disabled)"
            );
    ...
    function geteventpoints() {
      ...
      if (!(location.href == eventboton.href)) {
        ...
        let spotpuntos = document.querySelectorAll(
    ...
    function existevent() {
      let captureeventbutton = document.evaluate(
        ".//div[contains(@id,'submenu2')]/a[contains(@class,'eyecatcher')]",
        document.body,
        null,
        9,
        null
        ...
        try {
          ...
          let time = getRemainingTime(
            document.querySelector(
              "div#content article table tbody tr td span.ticker"
            )
          );
    ...
    function autopray() {
      ...
      if (posicion >= 0) {
        prayurl =
          prayurl.slice(0, posicion) +
          "guildTemple" +
          prayurl.slice(posicion + 8);
      }
    ...
    function autoquest() {
      ...
      let questrestart = document.querySelectorAll(
        "a.quest_slot_button_restart"
      );
      let questcomplete = document.querySelectorAll(
      ...
      if (localizacion == questurl) {
        if (questrestart.length > 0) {
          ...
          let questtime = getRemainingTime(
            document.querySelector("#quest_header_cooldown > b > span")
          );
          setCookie("questcooldown", true, questtime);
        } else {
          let questdiv = document.querySelectorAll(
            "div.contentboard_slot_inactive"
          );
          for (let i = 0; i < questdiv.length; i++) {
            let questtype = questdiv[i].querySelector("div.quest_slot_icon")
              .style.backgroundImage;
            if (
              questtype ==
                'url("9379/img/ui/quest/icon_dungeon_inactive.jpg")' ||
              questtype == 'url("9379/img/ui/quest/icon_items_inactive.jpg")' ||
              questtype == 'url("9379/img/ui/quest/icon_combat_inactive.jpg")'
            ) {
              let questaccept = questdiv[i].querySelectorAll(
    ...
    function createworkselect(autoworktype) {
      ...
      if (autoworktype == 0) {
        ...
        autoworktime.innerHTML =
          '<option value="1">1 HOUR</option><option value="2">2 HOURS</option><option value="3">3 HOURS</option><option value="4" selected>4 HOURS</option>';
      } else if (autoworktype == 2) {
        autoworktime.innerHTML =
          '<option value="1">1 HOUR</option><option value="2">2 HOURS</option><option value="3">3 HOURS</option><option value="4">4 HOURS</option><option value="5">5 HOURS</option><option value="6">6 HOURS</option><option value="7">7 HOURS</option><option value="8" selected>8 HOURS</option>';
      } else if (autoworktype == 3) {
        autoworktime.innerHTML =
          '<option value="1">1 HOUR</option><option value="2">2 HOURS</option><option value="3">3 HOURS</option><option value="4">4 HOURS</option><option value="5">5 HOURS</option><option value="6" selected>6 HOURS</option>';
      } else if (autoworktype == 4) {
        autoworktime.innerHTML =
          '<option value="1">1 HOUR</option><option value="2">2 HOURS</option><option value="3" selected>3 HOURS</option>';
      } else if (autoworktype == 5) {
        autoworktime.innerHTML =
          '<option value="4">4 HOURS</option><option value="5">5 HOURS</option><option value="6">6 HOURS</option><option value="7">7 HOURS</option><option value="8">8 HOURS</option><option value="9">9 HOUR</option><option value="10"selected>10 HOURS</option>';
      } else if (autoworktype == 7) {
        autoworktime.innerHTML =
    ...
    function main() {
      ...
      selectpraygod.innerHTML =
        '<option value="1" selected>Minerva</option><option value="2">Diana</option><option value="3">Vulcano</option><option value="4">Marte</option><option value="5">Apolo</option><option value="6">Mercurio</option>';
      selectpraygrace.id = "praygrace";
      selectpraygrace.innerHTML =
        '<option value="1" selected>1 grace</option><option value="2">2 grace</option><option value="3">3 grace</option><option value="4">4 grace</option><option value="5">5 grace</option><option value="6">6 grace</option><option value="7">7 grace</option><option value="8">8 grace</option><option value="9">9 grace</option><option value="10">10 grace</option><option value="11">11 grace</option><option value="12">12 grace</option><option value="13">13 grace</option><option value="14">14 grace</option><option value="15">15 grace</option><option value="16">16 grace</option><option value="17">17 grace</option><option value="18">18 grace</option>';
      if (autoprayok) {
      ...
      expdatalist.innerHTML =
        '<option value="5"></option><option value="10"></option><option value="15"></option><option value="20"></option><option value="25"></option><option value="30"></option><option value="35"></option><option value="40"></option><option value="45"></option><option value="50"></option><option value="55"></option><option value="60"></option><option value="65"></option><option value="70"></option><option value="75"></option><option value="80"></option><option value="85"></option><option value="90"></option><option value="95"></option><option value="100"></option>';
      ...
      if (autoexpeditionok) {
        ...
        selectexpeditionmap.setAttribute(
          "style",
          "display:none;margin-left:10px;"
        );
        selectexpeditiontarget.setAttribute(
          "style",
          "display:none;margin-left:10px;"
        );
        expeditionhp.setAttribute("style", "display:none;margin-left:10px;");
        expdatalabel.setAttribute(
          "style",
          "display:none;margin-left:10px;color:yellow;"
        );
      } else {
        ...
        expdatalabel.setAttribute(
          "style",
          "display:block;margin-left:10px;color:yellow;"
        );
      }
      ...
      selectexpeditiontarget.innerHTML =
      ...
      advanced.innerHTML =
        '<option value="false">NORMAL</option><option value="true">ADVANCED</option>';
      let skipboss = document.createElement("select");
      skipboss.id = "skipboss";
      skipboss.innerHTML =
        '<option value="false">KILL BOSS</option><option value="true">SKIP BOSS</option>';
      ...
      if (autoarenaok) {
        ...
        selectarenatarget.setAttribute(
          "style",
          "display:none;margin-left:10px;"
        );
        arenahp.setAttribute("style", "display:none;margin-left:10px;");
        arenadatalabel.setAttribute(
          "style",
          "display:none;margin-left:10px;color:yellow;"
        );
      } else {
      ...
      let lvl = parseInt(
        document.querySelector("div#header_values_level").innerHTML
      );
      selectarenatarget.innerHTML =
        '<option value="999" selected>No Limit</option><option value="' +
        (lvl - 3) +
        '">Target level <' +
        (lvl - 3) +
        '</option><option value="' +
      ...
      if (autoturmaok) {
        ...
        selectturmatarget.setAttribute(
          "style",
          "display:none;margin-left:10px;"
        );
      } else {
      ...
      if (autoeventok) {
        ...
        selecteventtarget.setAttribute(
          "style",
          "display:none;margin-left:10px;"
        );
        eventhp.setAttribute("style", "display:none;margin-left:10px;");
        eventdatalabel.setAttribute(
          "style",
          "display:none;margin-left:10px;color:yellow;"
        );
      } else {
      ...
      expeditionboton.addEventListener("click", function () {
        ...
        var selectedexpeditiontarget =
          document.querySelector("#expeditiontarget");
        var expdatalabel = document.querySelector("#expdatalabel");
        ...
        if (autoexpeditionok) {
          ...
          setCookie(
            "autoexpeditiontarget",
            selectedexpeditiontarget.value,
            10080
          );
    ...
    function getEventTime() {
      try {
        var timeText, x, time;
        timeText = document
          .querySelector("div#content span.ticker")
          .innerText.slice(-7);
        time = timeText[0] * 60 + timeText[2] * 10 + timeText[3] * 1 + 1;
        return time;
      } catch (error) {
    ...
    function hpLowerThan(hp) {
      let hpbar = document.querySelector(
        "div#header_values_hp_percent"
      ).innerText;
      let hpnumber = "";
      let index = 0;
