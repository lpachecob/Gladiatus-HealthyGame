...
class Notificaciones {
  static Rotativos() {
    ...
    InputNotificarOro.addEventListener("keypress", (input) => {
      if (input.key === "Enter") {
        if (
          !MontosGuardados.includes(InputNotificarOro.value) &&
    ...
    if (oroMaximo.value.length > 0) {
      ...
      for (let isItem = 0; isItem < item.length; isItem++) {
        if (item[isItem].hasAttribute("width") == true) {
          let auction_bid_div =
            item[isItem].children[1].children[0].children[7].children;
          let auction_item_div =
            item[isItem].children[1].children[0].children[6].children[1]
              .children[0];
